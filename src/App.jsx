...
export default function App() {
  ...
  const handleCloseModal = () => {
    setMaximizedView(null); setSearchProduct(''); setSearchOption(''); setSortConfig({ key: 'product', direction: 'asc' });
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-gray-50 text-gray-500 font-bold">DB 데이터를 불러오는 중입니다...</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 p-3 md:p-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><Carrot className="text-orange-500 shrink-0" />당근 판매 & 재고 관리 표</h1>
        ...
        <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200 w-full md:w-auto">
          <div className="flex items-center justify-between w-full md:w-auto gap-2">
            <span className="font-medium text-gray-700 whitespace-nowrap">{startDate !== endDate ? "조회 기간:" : "기준일:"}</span>
            ...
            <button onClick={handleTodayClick} className="ml-1 px-3 py-1 text-xs rounded-md border transition-colors bg-white border-gray-300 text-gray-700 hover:bg-gray-50 font-medium shadow-sm whitespace-nowrap">오늘</button>
          </div>
        </div>
      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 md:col-span-2 md:grid-cols-2">
                    <div>
                      ...
                      <input type="number" inputMode="numeric" pattern="[0-9]*" min="1" value={formData.quantity} onChange={handleFormChange} className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-violet-500 outline-none transition text-right" required />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">단가 (원)</label>
                      <input type="number" inputMode="numeric" pattern="[0-9]*" value={formData.price} onChange={handleFormChange} className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-violet-500 outline-none transition text-right" required />
                    </div>
                  </div>
                  <div className="md:col-span-2">
          ...
          <div className="lg:col-span-8">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden h-full flex flex-col">
              ...
              {summaryTab === 'settlement' && (
                <div className="flex-1 p-4 md:p-5 bg-white overflow-y-auto">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    {[
                      ...
                      <div key={item.key} className="border border-gray-200 bg-gray-50/50 rounded-xl p-4 shadow-sm">
                        <div className="mb-3">
                        ...
                        <div className="flex items-center">
                          <input type="number" inputMode="numeric" pattern="[0-9]*" value={settlementData[item.key] === 0 ? '' : settlementData[item.key]} onChange={(e) => setSettlementData({ ...settlementData, [item.key]: Number(e.target.value) })} className="w-full text-right font-bold text-lg border-b-2 border-gray-300 focus:border-indigo-500 outline-none bg-transparent py-1 transition-colors" placeholder="0" />
                  ...
                  <div className="flex justify-end mb-6">
                    <button onClick={handleSaveSettlement} disabled={isSettlementSaving} className="w-full md:w-auto bg-gray-800 hover:bg-gray-900 text-white px-6 py-3 md:py-2.5 rounded-lg font-bold text-sm shadow-sm transition-colors flex items-center justify-center gap-2 disabled:bg-gray-400 whitespace-nowrap break-keep">
                      <Database size={16} className="shrink-0 text-gray-300" /> <span>{isSettlementSaving ? "저장 중..." : "정산 현황 DB 저장"}</span>
                    </button>
                  </div>
                    ...
                    return (
                      <div className="flex flex-col gap-3">
                        <div className="p-4 bg-gray-50/50 rounded-xl border border-gray-200 flex flex-col md:flex-row justify-between md:items-center shadow-sm gap-2">
                          <div>
                            <span className="font-bold text-gray-800 flex items-center text-sm"><Clock size={16} className="text-red-600 mr-1.5" /> 미정산금 (내가 보관 중인 판매대금)</span>
                            <span className="text-xs text-gray-500 mt-1 block">누적 판매금({totalSales.toLocaleString()}) - 대표님 송금액({remitted.toLocaleString()})</span>
                          </div>
                          <span className="font-black text-xl text-red-600 text-right">{unsettledAmount.toLocaleString()} 원</span>
                        </div>
                        <div className="p-4 bg-gray-50/50 rounded-xl border border-gray-200 flex flex-col md:flex-row justify-between md:items-center shadow-sm gap-2">
                          <div>
                            <span className="font-bold text-gray-800 flex items-center text-sm"><Wallet size={16} className="text-indigo-600 mr-1.5" /> 현재 보유 자산</span>
                            <span className="text-xs text-gray-500 mt-1 block">내 계좌 잔액 + 보유중인 현금</span>
                          </div>
                          <span className="font-black text-xl text-indigo-600 text-right">{totalAsset.toLocaleString()} 원</span>
                        </div>
                        <div className="p-4 bg-gray-50/50 rounded-xl border border-gray-200 flex flex-col md:flex-row justify-between md:items-center shadow-sm gap-2">
                          <div>
                            <span className="font-bold text-gray-800 flex items-center text-sm"><Scale size={16} className={`mr-1.5 ${tillDifference === 0 ? 'text-emerald-600' : tillDifference > 0 ? 'text-blue-600' : 'text-orange-600'}`} /> 시재 (차액)</span>
                            <span className="text-xs text-gray-500 mt-1 block">현재 보유 자산 - 미정산금</span>
                          </div>
                          <span className={`font-black text-xl text-right ${tillDifference === 0 ? 'text-emerald-600' : tillDifference > 0 ? 'text-blue-600' : 'text-orange-600'}`}>{tillDifference > 0 ? '+' : ''}{tillDifference.toLocaleString()} 원</span>
                        </div>
                      </div>
                    );
                  })()}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="bg-white rounded-xl shadow-sm border border-emerald-200 overflow-hidden relative">
              ...
              <div className="bg-emerald-50/50 px-4 py-2 text-[11px] sm:text-xs text-emerald-700 border-b border-emerald-100 text-center font-medium">
              ...
              <div className="p-0 overflow-x-auto">
                <table className="w-full text-sm text-left min-w-[280px]">
                  <thead className="text-[11px] text-gray-600 bg-gray-50/50">
                    <tr><th className="px-3 py-2 font-medium">상품명</th><th className="px-3 py-2 font-medium text-center whitespace-nowrap">수량</th><th className="px-3 py-2 font-medium text-right whitespace-nowrap">금액</th></tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {periodSalesSummary.list.length === 0 ? (<tr><td colSpan="3" className="text-center text-gray-400 py-4">판매 내역 없음</td></tr>) : (
                      periodSalesSummary.list.map((item, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-gray-50">
                            <td className="px-3 py-2"><div className="font-bold text-gray-800 leading-tight">{item.product}</div><div className="text-[10px] text-emerald-600/80 font-bold mt-0.5">비중: {percent}%</div></td>
                            <td className="px-3 py-2 text-center font-bold align-top">{item.quantity}</td><td className="px-3 py-2 text-right text-emerald-600 font-bold whitespace-nowrap align-top">{item.amount.toLocaleString()}원</td>
                          </tr>
                        );
                      })
                    )}
                  ...
                  <tfoot className="bg-emerald-50/50 font-bold border-t border-emerald-100">
                    <tr><td className="px-3 py-3 text-emerald-800 text-xs">기간 합계</td><td className="px-3 py-3 text-center text-emerald-800">{periodSalesSummary.totalQty}</td><td className="px-3 py-3 text-right text-emerald-700 whitespace-nowrap">{periodSalesSummary.totalAmount.toLocaleString()}원</td></tr>
                  </tfoot>
                </table>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-blue-200 overflow-hidden relative">
               ...
               <div className="p-0 overflow-x-auto">
                <table className="w-full text-sm text-left min-w-[280px]">
                  <thead className="text-[11px] text-blue-700 bg-blue-50/50">
                    <tr><th className="px-3 py-2 font-medium">상품명</th><th className="px-3 py-2 font-medium text-center whitespace-nowrap">수량</th><th className="px-3 py-2 font-medium text-right whitespace-nowrap">금액</th></tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {totalSalesSummary.list.length === 0 ? (<tr><td colSpan="3" className="text-center text-gray-400 py-4">데이터 없음</td></tr>) : (
                      totalSalesSummary.list.map((item, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-blue-50/50">
                            <td className="px-3 py-2"><div className="font-bold text-gray-800 leading-tight">{item.product}</div><div className="text-[10px] text-blue-600/80 font-bold mt-0.5">비중: {percent}%</div></td>
                            <td className="px-3 py-2 text-center font-bold align-top">{item.quantity}</td><td className="px-3 py-2 text-right text-blue-600 font-bold whitespace-nowrap align-top">{item.amount.toLocaleString()}원</td>
                          </tr>
                        );
                      })
                    )}
                  ...
                  <tfoot className="bg-blue-50/50 font-bold border-t border-blue-100">
                    <tr><td className="px-3 py-3 text-blue-800 text-xs">누적 합계</td><td className="px-3 py-3 text-center text-blue-800">{totalSalesSummary.totalQty}</td><td className="px-3 py-3 text-right text-blue-700 whitespace-nowrap">{totalSalesSummary.totalAmount.toLocaleString()}원</td></tr>
                  </tfoot>
                </table>
              </div>
            </div>
            ...
            {maximizedView === 'inventory' ? (
                  ...
                  <>
                    ...
                    <div className="flex-1 overflow-x-auto overflow-y-auto bg-white relative">
                      <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 sticky top-0 shadow-sm text-gray-600 border-b border-gray-200 z-10">
                          <tr>
                            <th className="px-3 sm:px-4 py-3 font-semibold w-[45%] md:w-[25%] cursor-pointer hover:bg-gray-100" onClick={() => requestSort('product')}><div className="flex items-center gap-1 text-[11px] sm:text-sm">상품명 <span className="md:hidden font-normal text-gray-400">/ 정보</span> {getSortIcon('product')}</div></th>
                            <th className="px-3 sm:px-4 py-3 font-semibold w-[20%] cursor-pointer hover:bg-gray-100 hidden md:table-cell" onClick={() => requestSort('option')}><div className="flex items-center gap-1 text-[11px] sm:text-sm">옵션명 {getSortIcon('option')}</div></th>
                            <th className="px-1 sm:px-4 py-3 font-semibold text-center w-[15%] cursor-pointer hover:bg-gray-100" onClick={() => requestSort('qty')}><div className="flex justify-center gap-1 text-[11px] sm:text-sm">총 수량 {getSortIcon('qty')}</div></th>
                            <th className="px-1 sm:px-4 py-3 font-semibold text-right w-[15%] cursor-pointer hover:bg-gray-100 hidden md:table-cell" onClick={() => requestSort('sellPrice')}><div className="flex justify-end gap-1 text-[11px] sm:text-sm">단가 {getSortIcon('sellPrice')}</div></th>
                            <th className="px-1 sm:px-4 py-3 font-semibold text-center w-[20%] md:w-[15%] cursor-pointer hover:bg-gray-100" onClick={() => requestSort('remainQty')}><div className="flex justify-center gap-1 text-[11px] sm:text-sm">남은수량 {getSortIcon('remainQty')}</div></th>
                            <th className="px-1 sm:px-4 py-3 font-semibold text-center w-[20%] md:w-[10%] text-[11px] sm:text-sm">관리</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                                ...
                                return (
                                  <tr key={item.id} className="bg-yellow-50/50">
                                    <td className="px-2 py-2">
                                      <input type="text" value={invEditForm.product} onChange={e => setInvEditForm({...invEditForm, product: e.target.value})} className="w-full border rounded px-2 py-1 text-xs font-bold mb-1" placeholder="상품명" />
                                      <input type="text" value={invEditForm.option} onChange={e => setInvEditForm({...invEditForm, option: e.target.value})} className="w-full border rounded px-2 py-1 text-xs mb-1 md:hidden" placeholder="옵션명" />
                                      <input type="number" inputMode="numeric" pattern="[0-9]*" value={invEditForm.sellPrice} onChange={e => setInvEditForm({...invEditForm, sellPrice: e.target.value})} className="w-full border rounded px-2 py-1 text-xs md:hidden" placeholder="단가" />
                                    </td>
                                    <td className="px-2 py-2 hidden md:table-cell"><input type="text" value={invEditForm.option} onChange={e => setInvEditForm({...invEditForm, option: e.target.value})} className="w-full border rounded px-2 py-1 text-xs" /></td>
                                    <td className="px-1 py-2 text-center"><input type="number" inputMode="numeric" pattern="[0-9]*" value={invEditForm.qty} min="0" onChange={e => setInvEditForm({...invEditForm, qty: e.target.value})} className="w-full min-w-[40px] max-w-[60px] mx-auto border rounded px-1 py-1 text-xs text-center font-bold" /></td>
                                    <td className="px-2 py-2 text-right hidden md:table-cell"><input type="number" inputMode="numeric" pattern="[0-9]*" value={invEditForm.sellPrice} min="0" onChange={e => setInvEditForm({...invEditForm, sellPrice: e.target.value})} className="w-full min-w-[60px] max-w-[80px] ml-auto border rounded px-2 py-1 text-xs text-right" /></td>
                                    <td className="px-1 py-2 text-center text-[10px] text-gray-400">자동계산</td>
                                    <td className="px-1 py-2 text-center">
                                      <div className="flex justify-center items-center gap-1 sm:gap-2"><button onClick={saveInvEdit} className="text-emerald-600 bg-emerald-50 p-1 sm:p-1.5 rounded shadow-sm border border-emerald-200"><Check size={14} /></button><button onClick={cancelInvEdit} className="text-red-500 bg-red-50 p-1 sm:p-1.5 rounded shadow-sm border border-red-200"><X size={14} /></button></div>
                                    </td>
                                  </tr>
                                );
                              }
                              return (
                                <tr key={item.id} className="hover:bg-orange-50/40 group transition-colors">
                                  <td className="px-3 sm:px-4 py-3">
                                    <div className="font-bold text-gray-800 text-[13px] sm:text-sm leading-tight break-keep">{item.product}</div>
                                    <div className="text-[11px] text-gray-500 mt-1 md:hidden truncate">{item.option}</div>
                                    <div className="text-[11px] text-gray-400 mt-0.5 md:hidden">{item.sellPrice.toLocaleString()}원</div>
                                  </td>
                                  <td className="px-3 sm:px-4 py-3 text-gray-600 text-xs sm:text-sm hidden md:table-cell">{item.option}</td>
                                  <td className="px-1 sm:px-4 py-3 text-center font-semibold text-gray-900 text-xs sm:text-sm">{item.qty}</td>
                                  <td className="px-1 sm:px-4 py-3 text-right text-gray-500 text-xs sm:text-sm hidden md:table-cell">{item.sellPrice.toLocaleString()}원</td>
                                  <td className="px-1 sm:px-4 py-3 text-center">
                                    <span className={`inline-flex px-2 py-1 rounded font-bold text-[10px] sm:text-xs ${item.remainQty === 0 ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>{item.remainQty}</span>
                                  </td>
                                  <td className="px-1 sm:px-4 py-3 text-center">
                    ...
                    <div className="px-4 sm:px-6 py-3 sm:py-4 border-t flex justify-between items-center bg-orange-50/80 text-orange-900 shrink-0 z-10 shadow-[0_-2px_5px_rgba(0,0,0,0.02)]">
                      ...
                      <div className="flex gap-4 sm:gap-8 text-xs sm:text-sm items-center">
                        <div>총 수량: <span className="font-bold text-sm sm:text-lg">{processedInventory.reduce((acc, curr) => acc + curr.qty, 0)}</span></div>
                        <div>남은 수량: <span className="font-bold text-orange-600 text-sm sm:text-lg">{processedInventory.reduce((acc, curr) => acc + curr.remainQty, 0)}</span></div>
                      </div>
                    </div>
                  </>
                )}
              ...
              <div className="flex-1 flex flex-col overflow-hidden bg-white">
                 ...
                 <div className="flex-1 overflow-x-auto overflow-y-auto bg-white p-0 relative">
                   <table className="w-full text-sm text-left">
                     <thead className="bg-gray-50 sticky top-0 shadow-sm font-bold text-gray-600 z-10 border-b border-gray-200">
                       <tr>
                         <th className="px-3 sm:px-4 py-3 cursor-pointer hidden md:table-cell md:w-[15%]" onClick={() => requestSort('date')}><div className="flex items-center gap-1 text-[11px] sm:text-sm">판매일 {getSortIcon('date')}</div></th>
                         <th className="px-3 sm:px-4 py-3 cursor-pointer w-[45%] md:w-[20%]" onClick={() => requestSort('product')}><div className="flex items-center gap-1 text-[11px] sm:text-sm">상품명 <span className="md:hidden font-normal text-gray-400">/ 정보</span> {getSortIcon('product')}</div></th>
                         <th className="px-3 sm:px-4 py-3 cursor-pointer hidden md:table-cell md:w-[15%]" onClick={() => requestSort('option')}><div className="flex items-center gap-1 text-[11px] sm:text-sm">옵션명 {getSortIcon('option')}</div></th>
                         <th className="px-1 sm:px-4 py-3 text-center cursor-pointer w-[15%] md:w-[10%]" onClick={() => requestSort('quantity')}><div className="flex justify-center gap-1 text-[11px] sm:text-sm">수량 {getSortIcon('quantity')}</div></th>
                         <th className="px-1 sm:px-4 py-3 text-right cursor-pointer w-[20%] md:w-[15%]" onClick={() => requestSort('totalPrice')}><div className="flex justify-end gap-1 text-[11px] sm:text-sm">금액 {getSortIcon('totalPrice')}</div></th>
                         <th className="px-3 sm:px-4 py-3 hidden md:table-cell md:w-[15%]"><div className="flex items-center gap-1 text-[11px] sm:text-sm">비고</div></th>
                         <th className="px-1 sm:px-4 py-3 text-center w-[20%] md:w-[10%] text-[11px] sm:text-sm">관리</th>
                       </tr>
                     </thead>
                     <tbody className="divide-y divide-gray-100">
                         ...
                         modalDetailedData.map(s => {
                            if (editingSaleId === s.id) {
                              return (
                                <tr key={s.id} className="bg-yellow-50/50">
                                  <td className="px-1 py-1 hidden md:table-cell"><CustomDatePicker startDate={editForm.date} onChange={(start) => setEditForm({...editForm, date: start})} className="border px-1 py-1 text-xs bg-white" isRangeMode={false} /></td>
                                  <td className="px-2 py-2">
                                    <input type="text" value={editForm.product} onChange={e => setEditForm({...editForm, product: e.target.value})} className="w-full border rounded px-2 py-1 text-xs font-bold mb-1" placeholder="상품명" />
                                    <input type="text" value={editForm.option} onChange={e => setEditForm({...editForm, option: e.target.value})} className="w-full border rounded px-2 py-1 text-xs mb-1 md:hidden" placeholder="옵션명" />
                                    <input type="text" value={editForm.note} onChange={e => setEditForm({...editForm, note: e.target.value})} className="w-full border rounded px-2 py-1 text-xs md:hidden" placeholder="비고" />
                                  </td>
                                  <td className="px-2 py-2 hidden md:table-cell"><input type="text" value={editForm.option} onChange={e => setEditForm({...editForm, option: e.target.value})} className="w-full border rounded px-2 py-1 text-xs" /></td>
                                  <td className="px-1 py-2 text-center"><input type="number" inputMode="numeric" pattern="[0-9]*" value={editForm.quantity} onChange={e => setEditForm({...editForm, quantity: e.target.value})} className="w-full min-w-[40px] max-w-[60px] mx-auto border rounded px-1 py-1 text-xs text-center font-bold" /></td>
                                  <td className="px-2 py-2 text-right"><input type="number" inputMode="numeric" pattern="[0-9]*" value={editForm.price} onChange={e => setEditForm({...editForm, price: e.target.value})} className="w-full min-w-[60px] max-w-[80px] ml-auto border rounded px-2 py-1 text-xs text-right" /></td>
                                  <td className="px-2 py-2 hidden md:table-cell"><input type="text" value={editForm.note} onChange={e => setEditForm({...editForm, note: e.target.value})} className="w-full border rounded px-2 py-1 text-xs" /></td>
                                  <td className="px-1 py-2 text-center">
                            ...
                            return (
                              <tr key={s.id} className={`group transition-colors ${maximizedView === 'period' ? 'hover:bg-emerald-50/40' : 'hover:bg-blue-50/40'}`}>
                                <td className="px-3 sm:px-4 py-3 text-gray-500 text-xs hidden md:table-cell">{s.date}</td>
                                <td className="px-3 sm:px-4 py-3">
                                  <div className="font-bold text-gray-800 text-[13px] sm:text-sm leading-tight break-keep">{s.product}</div>
                                  <div className="text-[11px] text-gray-500 mt-1 md:hidden truncate"><span className="text-blue-500 font-medium">{s.date.substring(5)}</span> | {s.option}</div>
                                  <div className="text-[10px] text-gray-400 mt-0.5 md:hidden truncate max-w-[150px]">{s.note}</div>
                                </td>
                                <td className="px-3 sm:px-4 py-3 text-gray-600 text-xs sm:text-sm break-keep hidden md:table-cell">{s.option}</td>
                                <td className="px-1 sm:px-4 py-3 text-center font-semibold text-gray-900 text-xs sm:text-sm">{s.quantity}</td>
                                <td className={`px-1 sm:px-4 py-3 text-right font-bold whitespace-nowrap text-xs sm:text-sm ${maximizedView === 'period' ? 'text-emerald-600' : 'text-blue-600'}`}>{s.totalPrice.toLocaleString()}</td>
                                <td className="px-3 sm:px-4 py-3 text-gray-500 text-[11px] sm:text-xs truncate hidden md:table-cell" title={s.note}>{s.note || '-'}</td>
                                <td className="px-1 sm:px-4 py-3 text-center">
                                  <div className="flex justify-center items-center gap-1.5 md:opacity-0 group-hover:opacity-100 transition-opacity">
                 ...
                 {(() => {
                    ...
                    return (
                      <div className={`px-4 sm:px-6 py-3 sm:py-4 border-t flex justify-between items-center ${maximizedView === 'period' ? 'bg-emerald-50/80 text-emerald-900' : 'bg-blue-50/80 text-blue-900'} shrink-0 z-10 shadow-[0_-2px_5px_rgba(0,0,0,0.02)]`}>
                        <div className="text-xs sm:text-sm font-medium"><span className="opacity-80">개당 평균가:</span> <span className="font-bold">{tQ > 0 ? Math.round(tA/tQ).toLocaleString() : 0}원</span></div>
                        <div className="flex gap-4 sm:gap-8 text-xs sm:text-sm items-center">
                          <div>총 수량: <span className="font-bold text-sm sm:text-lg">{tQ}개</span></div>
                          <div>총 금액: <span className="font-bold text-sm sm:text-lg">{tA.toLocaleString()}원</span></div>
                        </div>
                      </div>
                    );
                 })()}
